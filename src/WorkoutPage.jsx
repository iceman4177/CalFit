...
export default function WorkoutPage({ userData, onWorkoutLogged }) {
  ...
  const [flowMode, setFlowMode] = useState('default'); // default | generating | suggested | accepted
  ...
  const scrollBoxIntoView = useCallback((el, block = 'center') => {
    try {
      if (!el) return;
      ...
      const target = block === 'start'
        ? Math.max(0, top - topPad)
        : Math.max(0, top - Math.max(24, (vh - rect.height) / 2));
...
setNewExercise({
  ...
  // ✅ Meals-style cloud sync: as exercises change, upsert the SAME workout row to Supabase (debounced).
  // This guarantees you'll see a POST/UPSERT for workouts just like meals, and cross-device hydration works.
  const upsertTimerRef = useRef(null);
  const upsertInFlightRef = useRef(false);
  const lastUpsertSigRef = useRef('');

  const buildWorkoutSessionFromExercises = useCallback((exArr) => {
    const now = new Date();
    const startedAt = startedAtRef.current || now.toISOString();
    const endedAt = now.toISOString();
    ...
    const arr = Array.isArray(exArr) ? exArr : [];
    const totalRaw = arr.reduce((sum, ex) => sum + (Number(ex?.calories) || 0), 0);
    const total = Math.round(totalRaw * 100) / 100;
    ...
    return {
      ...
      // local history fields (used by History UI)
      date: todayDisplay,
      name: (arr[0]?.exerciseName) || 'Workout',
      exercises: arr.map(ex => ({
        name: ex.exerciseName,
        sets: ex.sets,
        reps: ex.reps,
        weight: ex.weight || null,
  ...
  const debouncedUpsertWorkoutToCloud = useCallback((nextExercises, { immediate = false } = {}) => {
    // Only sync when signed in (guests stay local-only)
    if (!user?.id) return;

    const arr = Array.isArray(nextExercises) ? nextExercises : [];
    const total = arr.reduce((s, ex) => s + (Number(ex?.calories) || 0), 0);

    // Do not upsert empty workouts (DB check constraint)
    if (!arr.length || total <= 0) return;

    const sig = JSON.stringify(arr.map(ex => [ex?.exerciseName || '', ex?.sets || '', ex?.reps || '', ex?.weight || '', Math.round((Number(ex?.calories) || 0) * 100) / 100]));
    if (!immediate && sig === lastUpsertSigRef.current) return;

    if (upsertTimerRef.current) {
      try { clearTimeout(upsertTimerRef.current); } catch (e) {}
    ...
    const run = async () => {
      ...
      try {
        const session = buildWorkoutSessionFromExercises(arr);
        const res = await saveWorkoutLocalFirst(session);

        // If it succeeded, mark the local entry as uploaded so History UI can label it as synced.
        if (res?.ok) {
          try {
            const cid = String(session?.client_id || '');
            const list = readWorkoutHistory();
            const idx = list.findIndex(s => String(s?.client_id || s?.id || '') === cid);
            if (idx >= 0) {
              const next = [...list];
              next[idx] = { ...next[idx], uploaded: true, __draft: false, __cloud: true };
        ...
        lastUpsertSigRef.current = sig;
      } catch (e) {
        // localFirst already enqueues on failure; this keeps UI stable
        console.warn('[WorkoutPage] debouncedUpsertWorkoutToCloud failed', e);
      } finally {
    ...
    if (immediate) run();
    else upsertTimerRef.current = setTimeout(run, 550);
  }, [user?.id, buildWorkoutSessionFromExercises, readWorkoutHistory, writeWorkoutHistory]);
  ...
  useEffect(() => {
    if (flowMode !== 'accepted' || cumulativeExercises.length === 0) return;
    const run = () => scrollBoxIntoView(sessionLogRef.current, 'start');
    const t1 = setTimeout(run, 40);
    const t2 = setTimeout(run, 220);
    const t3 = setTimeout(run, 420);
    return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); };
  }, [flowMode, cumulativeExercises.length, scrollBoxIntoView]);

  const handleSuggestedLoadingChange = useCallback((isLoading) => {
    if (isLoading && showSuggestCard) setFlowMode('generating');
  }, [showSuggestCard]);

  const handleSuggestionReady = useCallback(() => {
    setFlowMode('suggested');
    const run = () => scrollBoxIntoView(suggestRef.current, 'start');
    const t1 = setTimeout(run, 40);
    const t2 = setTimeout(run, 220);
    const t3 = setTimeout(run, 420);
    return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); };
...
>
  <CardContent sx={{ pb: 2, pt: 2, overflow: 'visible' }}>
    ...
    >
      ...
      {(flowMode === 'default' || flowMode === 'generating') && (
        <Button
...
{/* GENERATING focus view */}
{flowMode === 'generating' && showSuggestCard && (
  <Box ref={suggestRef} sx={{ mt: 3, maxWidth: 860, mx: 'auto' }}>
    <SuggestedWorkoutCard
      userData={userData}
      onAccept={handleAcceptSuggested}
      onLoadingChange={handleSuggestedLoadingChange}
      onSuggestionReady={handleSuggestionReady}
    />
  </Box>
)}

{/* SUGGESTED focus view */}
{flowMode === 'suggested' && showSuggestCard && (
  <Box ref={suggestRef} sx={{ mt: 3, maxWidth: 860, mx: 'auto' }}>
    <SuggestedWorkoutCard
      userData={userData}
      onAccept={handleAcceptSuggested}
    ...
    <Grid item xs={12} md={4}>
      <Stack spacing={2}>
        <Button
          ...
          sx={{ justifyContent: 'flex-start', px: 0, fontWeight: 800, fontSize: '1.125rem' }}
        >
          Load Past Workout
        </Button>
      </Stack>
    ...
    <Grid item xs={12} md={8}>
      <Stack spacing={2.5}>
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 6px 18px rgba(0,0,0,0.04)' }}>
          <Typography variant="h6" gutterBottom sx={{ fontWeight: 800 }}>Log an exercise</Typography>
        ...
        {!showSaunaSection && (
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant="outlined" onClick={handleAddSaunaClick} sx={{ borderRadius: 999, px: 4, py: 1.25, fontWeight: 700 }}>
              Add Sauna Session
            </Button>
          </Box>
        )}
...
{/* ACCEPTED focus view */}
{flowMode === 'accepted' && cumulativeExercises.length > 0 && (
  <Paper ref={sessionLogRef} variant="outlined" sx={{ mt: 3, p: 2, borderRadius: 2, border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 6px 18px rgba(0,0,0,0.04)', maxWidth: 900, mx: 'auto' }}>
    ...
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mt: 2 }}>
      <Button variant="contained" fullWidth onClick={() => setCurrentStep(3)} sx={{ fontWeight: 800 }}>Submit Workout</Button>
      <Button variant="outlined" fullWidth onClick={() => setFlowMode('default')}>Edit Workout</Button>
    </Stack>
  </Paper>
)}
